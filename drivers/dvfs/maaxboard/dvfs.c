...
#define CORE_CLOCK (IMX8MQ_CLK_A53_CG)
...
const dvfs_core_info_t core_list[IMX_NUM_CORES] = {
    { .core_id = 0, .clk_src_id = CORE_CLOCK, .op_point_tbl = imx_op_point_table },
    { .core_id = 1, .clk_src_id = CORE_CLOCK, .op_point_tbl = imx_op_point_table },
    { .core_id = 2, .clk_src_id = CORE_CLOCK, .op_point_tbl = imx_op_point_table },
    { .core_id = 3, .clk_src_id = CORE_CLOCK, .op_point_tbl = imx_op_point_table }
};
...
static inline int do_set_frequency(op_point_idx_t op_point_idx) {
    ...
    if (returned_freq != target_freq) {
        LOG_DVFS_DRIVER_ERR("Target freq %zuMHz != actual freq %zuMHz!\n",
                            target_freq / (1000000), returned_freq / (1000000));
        return 1;
    }
    return 0;
}
...
void init(void) {
    ...
    // HACK: set PLL to 2000MHz initially
    uint64_t returned_freq;
    int ret = sddf_clk_set_rate(CLK_DRIVER_CHANNEL, IMX8MQ_ARM_PLL,
                                (uint64_t) (2000*1000000), &returned_freq);
    if (ret) {
        LOG_DVFS_DRIVER_ERR("Failed to set PLL initially!\n");
    }
    ret = sddf_clk_get_rate(CLK_DRIVER_CHANNEL, IMX8MQ_ARM_PLL,
                                &returned_freq);
    LOG_DVFS_DRIVER("Set PLL to %zu\n", (uint64_t) (returned_freq / 1000000));

    ret = sddf_clk_set_rate(CLK_DRIVER_CHANNEL, IMX8MQ_CLK_A53_CORE,
                                (uint64_t) (2000*1000000), &returned_freq);
    if (ret) {
        LOG_DVFS_DRIVER_ERR("Failed to set IMX8MQ_CLK_A53 initially!\n");
    }
    ret = sddf_clk_get_rate(CLK_DRIVER_CHANNEL, IMX8MQ_CLK_A53_CORE,
                                &returned_freq);
    LOG_DVFS_DRIVER("Set PLL OUT to %zu\n", (uint64_t) (returned_freq / 1000000));
