...
i2c_state_func_t *i2c_state_table[NUM_STATES] = { 
    state_idle, state_req, state_sel_cmd,
...
static void irqs_enable(bool is_receiver) {
    ...
    if (is_receiver) {
        regs->ier = INT_COMP_MASK | INT_NACK_MASK | INT_TO_MASK | 
            INT_ARB_LOST_MASK | INT_DATA_MASK | INT_RX_OVF_MASK;
    } else {
...
static inline void i2c_write_addr(i2c_driver_data_t *data) {
    if (cmd_is_read(data->active_cmd)) {
        LOG_I2C_DRIVER("BEFORE initiating read from slave 0x%02x (ISR=0x%x STATUS=0x%x)\n",
                       i2c_curr_addr(data), regs->isr, regs->status);
    } else {
    ...
    if (cmd_is_read(data->active_cmd)) {
        LOG_I2C_DRIVER("Initiated read from slave 0x%02x (ISR=0x%x STATUS=0x%x)\n",
                       i2c_curr_addr(data), isr_immediate, status_immediate);
    } else {
...
static int setup_master(bool is_receiver, bool repeated_start)
{
    ...
    ctrl |= I2C_CTRL_ACK_EN | I2C_CTRL_CLR_FIFO |
            I2C_CTRL_NA_MODE | I2C_CTRL_MASTER_MODE;
    ...
    LOG_I2C_DRIVER("setup_master(%s, %s): CTRL=0x%08x\n",
                   is_receiver ? "RX" : "TX",
                   repeated_start ? "repeated" : "simple",
                   regs->ctrl);
    return 0;
}
...
{
    uint32_t to_read = (byte_count > ZYNQ_MAX_TF_SIZE)
                        ? ZYNQ_MAX_TF_SIZE
                        : byte_count;
...
int i2c_set_sclk(uint32_t clock_hz)
{
	 ...
	 */
	uint32_t TempLimit = (((input_clock) %
    ...
    ctrl |= ((BestDivA & I2C_CTRL_DIV_A_MAX) << I2C_CTRL_DIV_A_OFFSET) | ((BestDivB & I2C_CTRL_DIV_B_MAX) << I2C_CTRL_DIV_B_OFFSET);
    regs->ctrl = ctrl;
    ...
    LOG_I2C_DRIVER("Clock configured: requested=%u Hz, actual=%u Hz (div_a=%u, div_b=%u)\n",
                   clock_hz, actual_freq, BestDivA, BestDivB);
...
void state_cmd(fsm_data_t *fsm, i2c_driver_data_t *data, i2c_queue_handle_t *queue_handle)
{
     ...
     */
    while ((cmd_is_read(data->active_cmd) || get_tf_fifo_space() > 0)
           && data->rw_idx < data->active_cmd.data_len) {
        ...
        if (data->await_wrrd) {
            ...
            if (data->await_wrrd == WRRD_WRADDR) {
                ...
                LOG_I2C_DRIVER("WRRD_WRADDR: data[0]=0x%02x, slave_addr=0x%02x\n",
                    cmd.payload.data[0], i2c_curr_addr(data));
...
void init(void)
{
    ...
    queue_handle = i2c_queue_init(config.virt.req_queue.vaddr, 
...
void notified(microkit_channel ch)
{
    ...
    if (driver_data.err != I2C_ERR_OK && 
        (fsm_data.curr_state == S_CMD || fsm_data.curr_state == S_CMD_RET)) {
        ...
        LOG_I2C_DRIVER_ERR("Spurious error interrupt! err=%u state=%s\n", 
                           driver_data.err, state_to_str(fsm_data.curr_state));
    }
}