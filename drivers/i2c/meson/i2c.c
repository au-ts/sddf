...
static inline void i2c_load_tokens(volatile struct i2c_regs *regs)
{
    ...
    regs->addr &= ~(0xff); // address and other bit is 0
    regs->addr |= ((i2c_ifState.addr & 0x7f) << 1);  // i2c hardware expects that the 7-bit address is shifted left by 1
    ...
    while (tk_offset < 16 && wdata_offset < 8 && rdata_offset < 8 && request_data_offset < i2c_ifState.curr_request_len) {
        ...
        if (request_data_offset == i2c_ifState.curr_request_len) { // we finshed putting all request data into registers
            if (tk_offset < 8) {
                ...
                regs->tk_list1 |= (MESON_I2C_TOKEN_END << ((tk_offset % 8) * 4));
            }
            tk_offset++;
            continue;
        }
        */
        ...
        if (meson_token == MESON_I2C_TOKEN_DATA && i2c_ifState.data_direction == DATA_DIRECTION_WRITE) {
            ...
            wdata_offset++;
            request_data_offset++; // Note it goes I2C_TOKEN_DATA -> buffer[i] in token chain so we need to increment here and then get incremented a few lines down for a total of twice
        }
...
static void handle_response_timeout(void)
{
    ...
    //     }
    //     if (i2c_ifState.curr_request_and_response_data) {
    //         releaseReqBuf(i2c_ifState.curr_request_and_response_data);
    //     }
    //     // i2c_ifState.curr_reseponse_data = NULL;
    //     // i2c_ifState.curr_request_and_response_data = 0x0;
    //     // i2c_ifState.curr_request_len = 0;
    //     // i2c_ifState.remaining = 0;
    //     // i2c_ifState.curr_reseponse_data = NULL;
    //     // i2c_ifState.curr_request_and_response_data = 0x0;
