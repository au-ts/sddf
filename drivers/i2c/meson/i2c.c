...
void state_req(fsm_data_t *fsm, i2c_driver_data_t *data)
{
    ...
    sddf_memcpy(&(data->curr_request), &header, sizeof(i2c_cmd_t));
    /*data->curr_request = header;*/
    data->err = I2C_ERR_OK;
...
void state_cmd(fsm_data_t *fsm, i2c_driver_data_t *data)
{
    ...
    //       send a STOP token.
    while (tk_offset < 16 && wdata_offset < 8 && rdata_offset < 8 && data->rw_idx <= data->active_cmd.data_len) {
        ...
        if (data->await_start) {
            ...
            // Increment to force us out of the transmission loop, even though we sent no data.
            data->rw_idx++;
