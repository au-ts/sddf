...
int regmap_multi_reg_write(uint64_t base, const struct reg_sequence *regs,
                           int num_regs)
{
    int i;
...
static void meson_clk_pll_init(struct clk *clk)
{
    ...
    if ((data->flags & CLK_MESON_PLL_NOINIT_ENABLED)
        && clk->hw.init->ops->is_enabled(clk))
        return;

    if (data->init_count) {
        /* Set the reset bit */
        regmap_update_bits(clk->base, data->rst.reg_off, data->rst.shift,
                           data->rst.width, 1);
                                               ...
                                               unsigned long parent_rate)
{
    ...
    n = regmap_read_bits(clk->base, data->n.reg_off, data->n.shift,
                         data->n.width);
    if (n == 0)
        return 0;

    m = regmap_read_bits(clk->base, data->m.reg_off, data->m.shift,
                         data->m.width);

    frac = data->frac.width
             ? regmap_read_bits(clk->base, data->frac.reg_off, data->frac.shift,
                                data->frac.width)
             : 0;
...
static int meson_clk_pll_is_enabled(struct clk *clk)
{
    ...
    if (!meson_parm_read(clk->base, data->en)
        || !meson_parm_read(clk->base, data->l)) {
        return 0;
    }
...
static int meson_clk_pll_enable(struct clk *clk)
{
    ...
    regmap_update_bits(clk->base, data->rst.reg_off, data->rst.shift,
                       data->rst.width, 1);
    regmap_update_bits(clk->base, data->en.reg_off, data->en.shift,
                       data->en.width, 1);
    regmap_update_bits(clk->base, data->rst.reg_off, data->rst.shift,
                       data->rst.width, 1);

    regmap_update_bits(clk->base, data->current_en.reg_off,
                       data->current_en.shift, data->current_en.width, 1);
    return 0;
}
                                      ...
                                      unsigned long prate)
{
    ...
    sdm = regmap_read_bits(clk->base, data->sdm.reg_off, data->sdm.shift,
                           data->sdm.width);
    n2 = regmap_read_bits(clk->base, data->n2.reg_off, data->n2.shift,
                          data->n2.width);
                         ...
                         uint32_t parent_rate)
{
    ...
    regmap_update_bits(clk->base, data->sdm.reg_off, data->sdm.shift,
                       data->sdm.width, sdm);
    regmap_update_bits(clk->base, data->n2.reg_off, data->n2.shift,
                       data->n2.width, n2);
...
static void mpll_init(struct clk *clk)
{
    ...
    regmap_update_bits(clk->base, data->sdm_en.reg_off, data->sdm_en.shift,
                       data->sdm_en.width, 1);
...
static unsigned long meson_vid_pll_div_recalc_rate(const struct clk *clk,
                                                   unsigned long prate)
{
    struct meson_vid_pll_div_data *data =
        (struct meson_vid_pll_div_data *)(clk->data);
    const struct vid_pll_div *div;
    uint32_t shift_val, shift_sel;

    shift_val = regmap_read_bits(clk->base, data->val.reg_off, data->val.shift,
                                 data->val.width);
    shift_sel = regmap_read_bits(clk->base, data->sel.reg_off, data->sel.shift,
                                 data->sel.width);
...
{
    struct meson_vclk_gate_data *data =
        (struct meson_vclk_gate_data *)(clk->data);

    regmap_update_bits(clk->base, data->enable.reg_off, data->enable.shift,
                       data->enable.width, 1);

    /* Do a reset pulse */
    regmap_update_bits(clk->base, data->reset.reg_off, data->reset.shift,
                       data->reset.width, 1);
    regmap_update_bits(clk->base, data->reset.reg_off, data->reset.shift,
                       data->reset.width, 0);
...
static int meson_vclk_gate_is_enabled(struct clk *clk)
{
    struct meson_vclk_gate_data *data =
        (struct meson_vclk_gate_data *)(clk->data);
    return regmap_read_bits(clk->base, data->enable.reg_off, data->enable.shift,
                            data->enable.width);
}
                                                ...
                                                unsigned long prate)
{
    ...
    uint32_t div = regmap_read_bits(clk->base, data->div.reg_off,
                                    data->div.shift, data->div.width);
                                   ...
                                   uint32_t parent_rate)
{
    ...
    return regmap_update_bits(clk->base, data->div.reg_off, data->div.shift,
                              data->div.width, div);
}
...
static unsigned long meson_clk_cpu_dyndiv_recalc_rate(const struct clk *clk,
                                                      unsigned long prate)
{
    struct meson_clk_cpu_dyndiv_data *data =
        (struct meson_clk_cpu_dyndiv_data *)(clk->data);
