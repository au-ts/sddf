...
static unsigned long mpll_recalc_rate(const struct clk *clk, unsigned long prate)
{
...
static int mpll_set_rate(const struct clk *clk, uint32_t rate, uint32_t parent_rate)
{
    struct meson_clk_mpll_data *data = (struct meson_clk_mpll_data *)(clk->data);
