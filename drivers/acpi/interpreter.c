        ...
        if (op_stage == PKT_LEN) {
            ...
            switch (op_code) {
                ...
                case LOCAL7_OP: {
                    ...
                    state_stack_add_argument((uintptr_t)local_node);
                    break;
                }
                case BYTE_PREFIX:
                case WORD_PREFIX:
                ...
                default: {
                    ...
                    if (node) {
                        ...
                        if (evaluation && node->op_code == METHOD_OP) {
                            ...
                            eval_namespace_node();
                            sddf_dprintf("come back from method %s eval\n", node->name);
...
{
    sddf_dprintf("start evaluating node\n");
    parse_namespace_node(true);
    sddf_dprintf("done\n");

    // TODO: clear ARG nodes
}
