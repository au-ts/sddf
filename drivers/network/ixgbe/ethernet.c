...
notified(microkit_channel ch)
{
    ...
    // sddf_dprintf("ECAM PBR: %x; MBR = %x\n", get_reg(ECAM + 0x64), get_reg(ECAM + 0x60));
    // sddf_dprintf("setting EIMS and EICS\n");
    // set_reg(EIMS, ~BIT(31));
    // set_reg(EICS, 1);
    // sddf_dprintf("ECAM PBR: %x\n", get_reg(ECAM + 0x64));
    // sddf_dprintf("PCI status reg: %x\n", get_reg16(ECAM + 0x6));
    ...
    case TX_CH:
        ...
        break;
    default:
        sddf_dprintf("ETH|LOG: received notification on unexpected channel: %X\n", ch);
        break;
    }
}
