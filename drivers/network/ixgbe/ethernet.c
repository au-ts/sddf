...
{
    return (ring->tail + 1) % ring->capacity == ring->head;
}
...
void rx_provide(void)
{
    ...
    while (reprocess) {
        ...
        while (!hw_ring_full(&rx) && !net_queue_empty_free(&rx_queue)) {
            ...
            volatile ixgbe_adv_rx_desc_t *desc = &rx_ring[rx.tail];
        ...
        if (provided) {
            /* Ensure all writes to the descriptor are ordered before we update the tail pointer
            * that makes hardware aware of the newly available slots */
            wwmb();
            eth_regs->rx_dma[0].rdt = rx.tail;
            eth_regs->rx_dma[0].rdt; // Read flush, make sure the above write complets
        }
...
static void rx_return(void)
{
    ...
    while (!hw_ring_empty(&rx)) {
        ixgbe_adv_rx_desc_wb_t desc = rx_ring[rx.head].wb;
...
void tx_provide(void)
{
    ...
    while (reprocess) {
        ...
        while (!(hw_ring_full(&tx)) && !net_queue_empty_active(&tx_queue)) {
            ...
            volatile ixgbe_adv_tx_desc_t *desc = &tx_ring[tx.tail];
        ...
        if (provided) {
            ...
            * that makes hardware aware of the newly available slots */
            wwmb();
            eth_regs->tx_dma[0].tdt = tx.tail;
            eth_regs->tx_dma[0].tdt; // Read flush, make sure the above write complets
        }
...
void tx_return(void)
{
    ...
    while (!hw_ring_empty(&tx)) {
        ...
        ixgbe_adv_tx_desc_wb_t hw_desc = tx_ring[tx.head].wb;

        if ((hw_desc.status & IXGBE_ADVTXD_STAT_DD) == 0)
            break;

        net_buff_desc_t descr_mdata = tx.descr_mdata[tx.head];
...
void init_1(void)
{
    ...
    // section 4.6.8 - init tx
    {
        ...
        //   - Bits[14:8]  hthresh: pre-fetch if at least `hthresh` unprocessed descriptors valid
        //   - Bits[22:16] wthresh: number of processed descriptors before write-back
...
void init_2(void)
{
    ...
    // sleep for 3 seconds. Just stabilize the hardware
