...
static inline void set_timeout_prescaler(short prescaler)
{
    ...
    short prescaler_masked = (prescaler & APBTIMER_CTRL_PRESCALER_MASK) << APBTIMER_CTRL_PRESCALER_SHIFT;
...
void init(void)
{
    ...
    for (int i = 0; i < device_resources.num_irqs; i++) {
        LOG_APBTIMER("IRQ %d = %u\n", i, device_resources.irqs[i].id);
    }
    LOG_APBTIMER("Timekeeper OVERFLOW is channel %u\n", OVERFLOW_IRQ(TIMER_TIMEKEEPER));
    LOG_APBTIMER("Timekeeper CMP is channel %u\n", CMP_IRQ(TIMER_TIMEKEEPER));
    LOG_APBTIMER("Timeout OVERFLOW is channel %u\n", OVERFLOW_IRQ(TIMER_TIMEOUT));
    LOG_APBTIMER("Timeout CMP is channel %u\n", CMP_IRQ(TIMER_TIMEOUT));
}
