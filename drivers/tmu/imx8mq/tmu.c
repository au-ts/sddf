...
// WARNING: IRQ forwarding is untested as the Maaxboard uses this IRQ channel for system management.
...
void init(void)
{
    ...
    assert(tmu_config_check_magic(&driver_config));
    regs = (volatile imx8mq_tmu_regs_t *)device_resources.regions[0].region.vaddr;
    enable_tmu();
}
...
microkit_msginfo protected(microkit_channel ch, microkit_msginfo msginfo)
{
    ...
    case SDDF_TMU_SET_IRQ_MODE:
        ...
        sddf_tmu_irq_modes_t new_mode = (sddf_tmu_irq_modes_t)microkit_mr_get(SDDF_TMU_SET_IRQ_MODE_MODE);
