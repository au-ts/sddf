...
const uint32_t SENSOR_COUNT = sizeof(SENSOR_INFO) / sizeof(SensorInfoEntry);
#endif

#define SDDF_THERMAL_GET_TEMPERATURE 0
#define SDDF_THERMAL_SET_ALARM_TEMPERATURE 1

// The temperature is in milli-Celsius
static inline int32_t sddf_thermal_get_temp(unsigned int channel, uint32_t sensor_index, uint32_t *temp_mc)
{
    ...
    if (!error) {
        *temp_mc = sddf_get_mr(1);
    }
    return error;
}

static inline uint32_t sddf_thermal_set_alarm(unsigned int channel, uint32_t sensor_index, int32_t temp_mc)
{
    sddf_set_mr(0, sensor_index);
    sddf_set_mr(1, temp_mc);
