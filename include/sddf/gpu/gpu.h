...
typedef struct gpu_req_resource_create_blob {
    ...
    uint32_t mem_size;
} gpu_req_resource_create_blob_t;

typedef struct gpu_req_set_scanout_blob {
    uint32_t resource_id;
    uint32_t scanout_id;
    /* Width in pixels of blob resource */
    uint32_t width;
    /* Height in pixels of blob resource */
    uint32_t height;
    /* Pixel format of blob resource */
    gpu_formats_t format;
    /* Bytes from one row of pixels to the next in blob resource rectangle */
    uint32_t stride;
    /* Offset into blob resource memory, where data to the blob resource rectangle begins */
    uint32_t offset;
    /* Rectangle within the blob resource for data to be scanned out from */
    gpu_rect_t rect;
...
typedef struct gpu_req_resource_create_2d {
    /* Assign resource with id */
    uint32_t resource_id;
    /* Width in pixels of resource */
    uint32_t width;
    /* Height in pixels of resource */
    uint32_t height;
    /* Pixel format of resource */
    gpu_formats_t format;
