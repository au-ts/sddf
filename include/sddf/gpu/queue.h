...
typedef enum gpu_req_code {
    ...
    /* Set the scanout of a 2D resource */
    GPU_REQ_SET_SCANOUT,
    /* Attach a contiguous memory backing to resource */
    GPU_REQ_RESOURCE_ATTACH_BACKING,
    /* Detach a contiguous memory backing from resource */
    GPU_REQ_RESOURCE_DETACH_BACKING,
    /* Transfer data from memory backing of a 2D resource to the resource's private memory */
    GPU_REQ_TRANSFER_TO_2D,
    /* Flush resource to scanout */
...
typedef struct gpu_req {
    ...
    union {
        ...
        gpu_req_resource_create_blob_t resource_create_blob;
