...
static void __libi2c_free_cmd(libi2c_conf_t *conf, i2c_cmd_t *cmd) {
    ...
    assert((uintptr_t) cmd > (uintptr_t)I2C_DATA_REGION && ((uintptr_t)cmd - (uintptr_t) I2C_DATA_REGION) <= i2c_config.data.size);
 ...
 */
static int __i2c_dispatch(libi2c_conf_t *conf, i2c_addr_t address, void *buf,
                       uint16_t len, uint8_t flag_mask) {
 ...
 * @return -1 if queue ops fail, positive value corresponding to i2c_err_t, or 0 if successful.
 */
int i2c_writeread(libi2c_conf_t *conf, i2c_addr_t address, i2c_addr_t reg_address, void *read_buf,
                  uint16_t len) {
    // Check that supplied buffer is within bounds of meta region
