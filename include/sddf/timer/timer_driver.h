...
static inline void timer_heap_heapify_down(timer_heap_t *heap, size_t index) {
    while (timer_heap_left_child(index) < heap->size) {
        ...
        if (right < heap->size &&
            heap->timeouts[right].timestamp < heap->timeouts[smallest].timestamp) {
            smallest = right;
        }
 ...
 */
static inline bool timer_heap_insert(timer_heap_t *heap, uint64_t timestamp, unsigned int client) {
    if (timer_heap_is_full(heap)) {
        return false;
    }

    heap->timeouts[heap->size].timestamp = timestamp;
    heap->timeouts[heap->size].client = client;
...
static inline bool timer_heap_pop(timer_heap_t *heap, timeout_t *result) {
    ...
    *result = heap->timeouts[0];
    ...
    if (heap->size > 0) {
        heap->timeouts[0] = heap->timeouts[heap->size];
