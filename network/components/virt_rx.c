...
typedef struct state {
    net_queue_handle_t rx_ring_drv;
    net_queue_handle_t rx_ring_clients[NUM_CLIENTS];
...
void rx_return(void)
{
    ...
    while (reprocess) {
        while (!net_queue_empty(state.rx_ring_drv.active)) {
            net_buff_desc_t buffer;
            int err = net_dequeue_active(&state.rx_ring_drv, &buffer);
            assert(!err);
            ...
            if (client >= 0) {
                err = net_enqueue_active(&state.rx_ring_clients[client], buffer);
                assert(!err);
        ...
        net_request_signal(state.rx_ring_drv.active);
        reprocess = false;

        if (!net_queue_empty(state.rx_ring_drv.active)) {
            net_cancel_signal(state.rx_ring_drv.active);
            reprocess = true;
        }
    }

    for (int client = 0; client < NUM_CLIENTS; client++) {
        if (notify_clients[client] && net_require_signal(state.rx_ring_clients[client].active)) {
            net_cancel_signal(state.rx_ring_clients[client].active);
...
void rx_provide(void)
{
    for (int client = 0; client < NUM_CLIENTS; client++) {
        bool reprocess = true;
        while (reprocess) {
            while (!net_queue_empty(state.rx_ring_clients[client].free)) {
                net_buff_desc_t buffer;
            ...
            net_request_signal(state.rx_ring_clients[client].free);
            reprocess = false;

            if (!net_queue_empty(state.rx_ring_clients[client].free)) {
                net_cancel_signal(state.rx_ring_clients[client].free);
                reprocess = true;
            }
        }
    }

    if (notify_drv && net_require_signal(state.rx_ring_drv.free)) {
        net_cancel_signal(state.rx_ring_drv.free);
...
void init(void)
{
    ...
    net_queue_init(&state.rx_ring_drv, (net_queue_t *)rx_free_drv, (net_queue_t *)rx_active_drv, RX_QUEUE_SIZE_DRIV);
    virt_queue_init_sys(microkit_name, state.rx_ring_clients, rx_free_arp, rx_active_arp);
    net_buffers_init((net_queue_t *)rx_free_drv, buffer_data_paddr, RX_QUEUE_SIZE_DRIV);

    if (net_require_signal(state.rx_ring_drv.free)) {
        net_cancel_signal(state.rx_ring_drv.free);
