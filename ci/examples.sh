...
timer() {
    BOARDS=("imx8mm_evk" "imx8mp_evk" "imx8mq_evk" "maaxboard" "odroidc2" "odroidc4" "qemu_virt_aarch64" "qemu_virt_riscv64" "star64" "zcu102")
    CONFIGS=("debug" "release")
    for BOARD in "${BOARDS[@]}"
    do
      for CONFIG in "${CONFIGS[@]}"
...
serial() {
    BOARDS=("imx8mm_evk" "imx8mq_evk" "imx8mp_evk" "maaxboard" "odroidc2" "odroidc4" "qemu_virt_aarch64" "qemu_virt_riscv64" "star64" "zcu102")
    CONFIGS=("debug" "release")
    for BOARD in "${BOARDS[@]}"
    do
      for CONFIG in "${CONFIGS[@]}"
