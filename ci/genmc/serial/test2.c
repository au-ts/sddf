...
#if PRODUCER * BATCH_SIZE < CONSUMER
...
void *producer(void *p)
{
    ...
    for (uint64_t i = 0; i < PRODUCER; i++) {
        for (uint64_t j = 0; j < BATCH_SIZE; j++) {
            while (serial_enqueue_local(&queue_handle, &local_tail, (char)(i * BATCH_SIZE + j)) != 0);
        }
        serial_update_shared_tail(&queue_handle, local_tail);
    }
    return NULL;
}
