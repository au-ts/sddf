...
void process_request(uint32_t client_id)
{
    ...
    while (!i2c_queue_empty(client_queues[client_id].request->ctrl) && !i2c_queue_full(driver_queue.request->ctrl)) {
        ...
        uint16_t len;
        int err = i2c_dequeue_request(client_queues[client_id], &bus_address, &data_start_vaddr, &meta_base_vaddr, &len);
        if (err) {
        ...
        if (offset > config.clients[client_id].data_size ||
            ...
            LOG_VIRT_ERR("invalid data vaddr (0x%lx) given by client %u.",
                         data_start_vaddr, client_id);
            continue;
        }
