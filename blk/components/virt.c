...
static bool handle_mbr_reply()
{
    ...
    sddf_vspace_invalidate_data(3, mbr_bk.vaddr, mbr_bk.vaddr + (BLK_TRANSFER_SIZE * mbr_bk.count));
#endif
        ...
        case BLK_REQ_READ:
            if (drv_status == BLK_RESP_OK) {
                ...
                /* TODO: This is a raw seL4 system call because Microkit does not (currently)
                    * include a corresponding libmicrokit API. */
#ifdef CONFIG_ARCH_ARM
                sddf_vspace_invalidate_data(3, reqbk.vaddr, reqbk.vaddr + (BLK_TRANSFER_SIZE * reqbk.count));
#endif
            }
            break;
        case BLK_REQ_WRITE:
