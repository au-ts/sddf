...
static uint32_t res_map_drv_to_virt[GPU_MAX_RESOURCES]; /* mapping from driver resource id to virtualiser resource id */

/* Mapping virtualiser scanout ids to driver scanout ids may not make sense when we have a proper windowing system,
 * but for now its "good enough" implementation of an emulated scanout.
 */
static ialloc_t so_ialloc; /* index allocator for driver scanout ids */
static uint32_t so_ialloc_idxlist[GPU_MAX_SCANOUTS];
static uint32_t so_map_virt_to_drv[GPU_MAX_SCANOUTS]; /* mapping from virtualiser scanout id to driver scanout id */
static uint32_t so_map_drv_to_virt[GPU_MAX_SCANOUTS]; /* mapping from driver scanout id to virtualiser scanout id */
...
{
    if (!initialised) {
        if (!gpu_queue_empty_config(&drv_h)) {
...
static inline bool res_virt_valid(uint32_t res_id)
{
    return res_map_virt_to_drv[res_id] != INVALID_RES_ID;
}

static inline uint32_t res_map(uint32_t virt_res_id, uint32_t drv_res_id)
{
    res_map_virt_to_drv[virt_res_id] = drv_res_id;
    res_map_drv_to_virt[drv_res_id] = virt_res_id;
}

static inline void res_unmap(uint32_t virt_res_id, uint32_t drv_res_id)
{
    res_map_virt_to_drv[virt_res_id] = INVALID_RES_ID;
    res_map_drv_to_virt[drv_res_id] = INVALID_RES_ID;
}
...
static inline uint32_t res_drv_to_virt(uint32_t drv_res_id)
{
    return res_map_drv_to_virt[drv_res_id];
}

static inline uint32_t so_map(uint32_t virt_so_id, uint32_t drv_so_id)
{
    so_map_virt_to_drv[virt_so_id] = drv_so_id;
    so_map_drv_to_virt[drv_so_id] = virt_so_id;
}
...
static void handle_client(int cli_id)
{
    ...
    while (!gpu_queue_empty_req(h)) {
        ...
        if (req.code == GPU_REQ_RESOURCE_CREATE_2D) {
        ...
        /* Other than GPU_REQ_RESOURCE_CREATE_2D, all other requests operate on an existing resource that needs to be valid */
        if (!res_virt_valid(req.resource_id)) {
            LOG_GPU_VIRT("Resource id not found, failing request");
        ...
        case GPU_REQ_RESOURCE_UNREF: {
            ...
            drv_req.id = drv_req_id;
            drv_req.resource_unref.resource_id = res_map_virt_to_drv[req.resource_unref.resource_id];
            ialloc_free(&res_ialloc, res_map_virt_to_drv[req.resource_unref.resource_id]);
            break;
        }
        case GPU_REQ_RESOURCE_ATTACH_BACKING: {
            drv_req.code = GPU_REQ_RESOURCE_ATTACH_BACKING;
            drv_req.id = drv_req_id;
            drv_req.resource_attach_backing.resource_id = res_map_virt_to_drv[req.resource_attach_backing.resource_id];
        ...
        case GPU_REQ_RESOURCE_DETACH_BACKING: {
            ...
            drv_req.id = drv_req_id;
            drv_req.resource_detach_backing.resource_id = res_map_virt_to_drv[req.resource_detach_backing.resource_id];
            break;
        }
        case GPU_REQ_SET_SCANOUT: {
            // TODO: REVISIT
            if (ialloc_full(&so_ialloc)) {
                LOG_GPU_VIRT("Scanout mapping full, failing request");
                goto req_fail;
            }
            uint32_t drv_so_id = 0;
            ialloc_alloc(&so_ialloc, &drv_so_id);
            ...
            drv_req.set_scanout.resource_id = res_map_virt_to_drv[req.set_scanout.resource_id];
        ...
        case GPU_REQ_TRANSFER_TO_2D: {
            ...
            drv_req.id = drv_req_id;
            drv_req.transfer_to_2d.resource_id = res_map_virt_to_drv[req.transfer_to_2d.resource_id];
        ...
        case GPU_REQ_RESOURCE_FLUSH: {
            ...
            drv_req.id = drv_req_id;
            drv_req.resource_flush.resource_id = res_map_virt_to_drv[req.resource_flush.resource_id];
            break;
        }
        default: {
