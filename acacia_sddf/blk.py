...
class BlkClientOptions:
    ...
    def __post_init__(self):
        if self.data_size % 2:
            raise ValueError("Blk client data regions must be a multiple of 2!")
...
class sDDFBlk(sDDFDriverClass):
    ...
    def construct_infrastructure(self, virt_prio: int):
        ...
        # used in blk/components/partitioning.c for the blk virtualiser to read
        # in the partition table
        self.virtualiser_data_mr = MemoryRegion(
            self.sdf, "virtualiser_data", self.driver_data_size, physical=True
        )
    ...
    def create_driver_virt_connection(self):
        ...
        driver_q_mr_sz = self.sdf.arch.roundup_to_page(
            driver_q_capacity * BLK_QUEUE_ENTRY_SZ
        )
        ...
        virt_data_map = self.virt.create_automap(
            self.virtualiser_data_mr, Map.Permissions(r=True, w=True)
        )
    ...
    def connect_clients(self):
        ...
        for c in self.clients:
            ...
            queue_mr_sz = self.sdf.arch.roundup_to_page(
                cfg.queue_capacity * BLK_QUEUE_ENTRY_SZ
            )
            ...
            data_mr_sz = self.sdf.arch.roundup_to_page(cfg.data_size)
            data_mr = MemoryRegion(
                self.sdf, f"blk_client_{c.name}_data", data_mr_sz, physical=True
            )
    ...
    def blk_connection_resource_factory(
        ...
        return ConfigStruct(fields, "blk_connection_resource_t")
