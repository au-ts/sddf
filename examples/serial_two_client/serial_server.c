...
int serial_server_printf(char *string) {
    ...
    int ret = serial_dequeue_free(&local_server->tx_queue, &buffer_offset, &buffer_len);
    ...
    uintptr_t buffer = tx_data + buffer_offset;
    ...
    ret = serial_enqueue_active(&local_server->tx_queue, buffer_offset, print_len);
...
int getchar() {
    ...
    while (serial_dequeue_active(&local_server->rx_queue, &buffer_offset, &buffer_len) != 0) {
    ...
    uintptr_t buffer = rx_data + buffer_offset;
    ...
    int ret = serial_enqueue_free(&local_server->rx_queue, buffer_offset, buffer_len);
...
void init(void) {
    ...
    for (int i = 0; i < NUM_ENTRIES - 1; i++) {
        int ret = serial_enqueue_free(&local_server->rx_queue, i * BUFFER_SIZE, BUFFER_SIZE);
    ...
    for (int i = 0; i < NUM_ENTRIES - 1; i++) {
        ...
        int ret = serial_enqueue_free(&local_server->tx_queue, ((i + NUM_ENTRIES) * BUFFER_SIZE), BUFFER_SIZE);
