...
static struct blk_client_config config;
...
bool test_basic()
{
    ...
    case START: {
        ...
        char *data_dest = (char *)config.data;
    ...
    case FINISH: {
        ...
        char *read_data = (char *)(config.data + 0x10000);
...
void init(void)
{
    ...
    sddf_memcpy(&config, client_data, client_data_len);

    blk_queue_init(&blk_queue, config.req_queue, config.resp_queue, config.queue_capacity);

    /* Want to print out configuration information, so wait until the config is ready. */
    while (!blk_storage_is_ready(config.storage_info));
    LOG_CLIENT("device config ready\n");

    LOG_CLIENT("device size: 0x%lx bytes\n", config.storage_info->capacity * BLK_TRANSFER_SIZE);
