...
def generate(sdf_file: str, output_dir: str, dtb: DeviceTree | None, need_timer: bool):
    ...
    if board.arch == SystemDescription.Arch.X86_64:
        blk_requests_mr = SystemDescription.MemoryRegion(sdf, "virtio_requests", 65536, paddr=0x5fdf0000)
        ...
        blk_virtio_metadata_mr = SystemDescription.MemoryRegion(sdf, "virtio_metadata", 65536, paddr=0x5fff0000)
        sdf.add_mr(blk_virtio_metadata_mr)
        blk_virtio_metadata_map = SystemDescription.Map(blk_virtio_metadata_mr, 0x20210000, "rw")
        blk_driver.add_map(blk_virtio_metadata_map)

        virtio_blk_regs = SystemDescription.MemoryRegion(sdf, "virtio_net_regs", 0x4000, paddr=0xfe000000)
        sdf.add_mr(virtio_blk_regs)
        virtio_blk_regs_map = SystemDescription.Map(virtio_blk_regs, 0x6000_0000, "rw", cached=False)
        blk_driver.add_map(virtio_blk_regs_map)

        virtio_blk_irq = SystemDescription.IrqIoapic(board.arch, ioapic_id=0, pin=11, vector=1, id=17)
        blk_driver.add_irq(virtio_blk_irq)

        pci_config_addr_port = SystemDescription.IoPort(SystemDescription.Arch.X86_64, 0xCF8, 4, 1)
