...
IPERF_CLIENT := ${SDDF}/examples/iperf_client
...
SYSTEM_FILE := iperf_client.system
IMAGE_FILE := loader.img
REPORT_FILE := report.txt

include ${SDDF}/tools/make/board/common.mk

METAPROGRAM := $(IPERF_CLIENT)/meta.py
LWIPDIR := network/ipstacks/lwip/src
BENCHMARK := $(SDDF)/benchmark
UTIL := $(SDDF)/util
ETHERNET_DRIVER := $(SDDF)/drivers/network/$(NET_DRIV_DIR)
ETHERNET_CONFIG_INCLUDE := ${IPERF_CLIENT}/include/ethernet_config
...
vpath %.c ${SDDF} ${IPERF_CLIENT}

IMAGES := eth_driver.elf client.elf benchmark.elf idle.elf \
...
client.elf: $(IPERF_OBJS) libsddf_util.a lib_sddf_lwip_iperf_client.a
	$(LD) $(LDFLAGS) $^ $(LIBS) -o $@
...
$(SYSTEM_FILE): $(METAPROGRAM) $(IMAGES) $(DTB)
	...
	$(OBJCOPY) --update-section .net_copy_config=net_copy_iperf_net_copier.data network_copy.elf
	$(OBJCOPY) --update-section .device_resources=timer_driver_device_resources.data timer_driver.elf
	$(OBJCOPY) --update-section .timer_client_config=timer_client_iperf.data client.elf
	$(OBJCOPY) --update-section .net_client_config=net_client_iperf.data client.elf
	$(OBJCOPY) --update-section .serial_client_config=serial_client_iperf.data client.elf
	$(OBJCOPY) --update-section .lib_sddf_lwip_config=lib_sddf_lwip_config_iperf.data client.elf
	touch $@
...
# LIB_SDDF_LWIP_SRCS += $(LWIPDIR)/apps/lwiperf/lwiperf.c
# include ${SDDF}/network/ipstacks/lwip/src/Filelists.mk
