...
void netif_status_callback(char *ip_addr)
{
    ...
    if (!iperf3_started) {
        ...
        sddf_printf("Connecting to server at %s\n", ip4addr_ntoa(&server_addr));
...
void on_timer_event(struct udp_pcb *pcb) {
  ...
  set_timeout(); // ideally arm to min(next_lwip_deadline, tx->end_us if running)
}
