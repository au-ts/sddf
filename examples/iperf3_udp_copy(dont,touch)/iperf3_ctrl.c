      ...
      while (i < n) {
        if (ctrl->rx_phase == CTRL_WAIT_STATE) {
          ...
          if (st == PARAM_EXCHANGE) {
            ...
            ctrl->num_streams = 1; // hardcoded for now

            bool udp = true;
            if (udp) {
              // Create UDP streams instead of TCP streams
              for (int s = 0; s < ctrl->num_streams; s++) {
                ...
                // Allocate pbuf for cookie
                struct pbuf *p = pbuf_alloc(PBUF_TRANSPORT, sizeof(uint32_t), PBUF_RAM);
                if (!p) {
                    sddf_printf("pbuf alloc failed\n");
                    continue;
                }

                // Copy cookie into payload
                uint32_t msg = htonl(UDP_CONNECT_MSG);
                memcpy(p->payload, &msg, sizeof(msg));

                // Send cookie
                err_t err = udp_sendto(udp_stream->pcb, p, &udp_stream->server_addr, udp_stream->server_port); 
            ...
            for (int s = 0; s < MAX_STREAMS; s++) {
                ...
                if (stream->pcb != NULL) {
                    // set stream to send payload now
                    stream->phase = SEND_PAYLOAD;
                    stream->burst_max = 9999999999999;
                    stream->tx_buf = ctrl->payload; // set to payload buffer
...
void iperf3_on_timer_tick(iperf_ctrl_t *ctrl, uint32_t now_ms) {
    // 1) update ctrl->test_done / ctrl->omitting

    if (ctrl->test_active && !ctrl->test_done && now_ms >= ctrl->end_time_ms) {
        ctrl->test_done = true;
    }
    // 2) if UDP running: pump udp streams
    if (ctrl->test_active && !ctrl->test_done) {
        // sddf_printf("[tick] now=%u end=%u active=1\n", now_ms, ctrl->end_time_ms);
        for (int s = 0; s < ctrl->num_streams; s++) {
            iperf3_udp_stream_t *stream = &ctrl->udp_streams[s];
            // sddf_printf("[tick] stream=%d pcb=%p phase=%d burst=%u\n", s, stream->pcb, (int)stream->phase, stream->burst_max);
    ...
    // 3) if test ended: stop streams + send TEST_END on ctrl TCP
