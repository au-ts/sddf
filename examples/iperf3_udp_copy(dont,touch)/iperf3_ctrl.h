#ifndef IPERF3_CTRL_H
#define IPERF3_CTRL_H

#include <stdint.h>
#include <stdbool.h>
#include "lwip/tcp.h"
#include "lwip/ip_addr.h"

#include "iperf3_stream.h"

#define IPERF3_COOKIE_LEN 37
#define MAX_STREAMS 15
#define PAYLOAD_SIZE 16384

typedef enum { 
  CTRL_WAIT_STATE, // 1 BYTE STATE FROM SERVER
  CTRL_WAIT_JSON_LEN, // for waiting for the 4 byte json length field from server
  CTRL_WAIT_JSON_BODY, // for waiting for the json body from server
} ctrl_rx_phase_t;

typedef struct iperf_ctrl {
  struct tcp_pcb *pcb;
  uint8_t cookie[IPERF3_COOKIE_LEN];

  // iperf3 implements number of streams as a linked list
  iperf3_stream_t streams[MAX_STREAMS];
  iperf3_udp_stream_t udp_streams[MAX_STREAMS];

  
  // uint8_t cookie_rx;
  ctrl_rx_phase_t rx_phase;
  uint8_t server_state;
  uint32_t json_len;
  uint32_t json_rx;

  uint8_t json_len_buf[4];
  uint8_t json_len_rx;
  uint8_t payload[PAYLOAD_SIZE];
  uint8_t result_json[65536];

  // num streams to use in test
  uint8_t num_streams;

  uint64_t bytes_sent;

  // tx state
  const int8_t *tx_buf;
  uint16_t tx_len;
  uint16_t tx_off;

  // check if test done
  bool test_done;

  // check if test active (started but not done)
  bool test_active;
  bool sent_test_end; // to check if we've sent the TEST_END state to server

  // omitting
  uint32_t omit_ms;
  uint32_t omit_end_ms;
  bool omitting;


  uint32_t duration_ms; // duration excluding omit time
  uint32_t end_time_ms; // absolute end time in ms since boot when test should end
  //timers??

  // benchmark_client_config_t bench;
  // bool bench_active;
} iperf_ctrl_t;

void iperf3_ctrl_init(iperf_ctrl_t *ctrl);
err_t iperf_ctrl_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err);
err_t iperf_ctrl_sent(void *arg, struct tcp_pcb *tpcb, u16_t len);
void iperf3_ctrl_err(void *arg, err_t err);
err_t iperf_ctrl_connect(void *arg,struct tcp_pcb *pcb, err_t err);
void iperf3_on_timer_tick(iperf_ctrl_t *ctrl, uint32_t now_ms);

#endif