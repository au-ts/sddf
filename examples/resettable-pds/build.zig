...
pub fn build(b: *std.Build) !void {
   ...
   const faulter = b.addExecutable(.{
    ...
    faulter.addCSourceFile(.{ .file = b.path("faulter.c") });
    faulter.addIncludePath(sddf_dep.path("include"));
    faulter.addIncludePath(sddf_dep.path("include/microkit"));
    faulter.linkLibrary(sddf_dep.artifact("util"));
    faulter.linkLibrary(sddf_dep.artifact("util_putchar_serial"));

    faulter.addIncludePath(libmicrokit_include);
    faulter.addObjectFile(libmicrokit);
    faulter.setLinkerScript(libmicrokit_linker_script);

    b.installArtifact(faulter);
   ...
   const parent = b.addExecutable(.{
    ...
    parent.linkLibrary(sddf_dep.artifact("util_putchar_serial"));

    parent.addIncludePath(libmicrokit_include);
    parent.addObjectFile(libmicrokit);
    parent.setLinkerScript(libmicrokit_linker_script);

    b.installArtifact(parent);
    ...
    microkit_tool_cmd.step.dependOn(&elf_objcopy.step);
