...
def generate(sdf_file: str, output_dir: str, dtb: DeviceTree, get_core: Callable[[str], int]):
    ...
    client0 = ProtectionDomain("client0", "echo0.elf", priority=97, budget=20000, cpu=get_core("client0"))
    ...
    copy_elf("echo", "echo", 1)
    client1 = ProtectionDomain("client1", "echo1.elf", priority=97, budget=20000, cpu=get_core("client1"))
    ...
    for i in range(num_cores):
        ...
        core_objs[i]["idle"].add_map(Map(cycle_counters_mr, 0x5_000_000, perms=Map.Perms(r=True, w=True)))
        client0.add_map(Map(cycle_counters_mr, 0x20_000_000 + 0x1000 * i, perms=Map.Perms(r=True, w=False)))
