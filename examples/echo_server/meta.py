...
# fix lambda type
def generate(sdf_file: str, output_dir: str, dtb: DeviceTree, get_core):
    ...
    client0 = ProtectionDomain("client0", "lwip0.elf", priority=97, budget=20000, cpu=get_core("client0"))
    client0_net_copier = ProtectionDomain(
        "client0_net_copier", "network_copy0.elf", priority=98, budget=20000, cpu=get_core("client0_net_copier")
    )
    client1 = ProtectionDomain("client1", "lwip1.elf", priority=97, budget=20000, cpu=get_core("client1"))
    client1_net_copier = ProtectionDomain(
        "client1_net_copier", "network_copy1.elf", priority=98, budget=20000, cpu=get_core("client1_net_copier")
    )
    ...
    for pd in child_pds:
        ...
        if core in get_core_pds:
            get_core_pds[core].append(pd)
        else:
            get_core_pds[core] = [pd]
    ...
    idle_configs = []
    bench_configs = []
    for core in sorted(get_core_pds):
        cores.append(core)

        # Create bench and idle pds for each active core
        idle = ProtectionDomain("bench_idle" + str(core), "idle" + str(core) + ".elf", priority=1, cpu=core)
        sdf.add_pd(idle)

        bench = ProtectionDomain("bench" + str(core), "benchmark" + str(core) + ".elf", priority=254, cpu=core)
        sdf.add_pd(bench)
        ...
        children = []
        for pd in get_core_pds[core]:
            child_id = bench.add_child_pd(pd)
            children.append((child_id, pd.name))

        # Benchmark idle channels
        init_ch = Channel(idle, bench)
        ...
        if prev_bench_pd == 0:
            ...
            start_ch = Channel(prev_bench_pd, bench)
            stop_ch = Channel(prev_bench_pd, bench)
        ...
        idle.add_map(Map(cycle_counters_mr, 0x5_000_000, perms=Map.Perms(r=True, w=True)))
        client_vaddr = 0x20_000_000 + 0x1000 * (len(cores) - 1)
        client0.add_map(Map(cycle_counters_mr, client_vaddr, perms=Map.Perms(r=True, w=False)))

        # Create configs
        idle_configs.append( BenchmarkIdleConfig(0x5_000_000, init_ch.pd_a_id))
        if prev_bench_pd != 0:
            bench_configs.append(BenchmarkConfig(
                prev_start_ch.pd_b_id,
                start_ch.pd_a_id,
                prev_stop_ch.pd_b_id,
                stop_ch.pd_a_id,
                prev_init_ch.pd_b_id,
                cores[-2],
                False,
                prev_children
            ))
            ...
            bench_client_config = BenchmarkClientConfig(
                ...
                stop_ch.pd_a_id,
                len(get_core_pds),
                list(((0x20_000_000 + 0x1000 * i) for i in range(len(get_core_pds))))
            )

        prev_bench_pd = bench
        prev_init_ch = init_ch
        prev_start_ch = start_ch
        prev_stop_ch = stop_ch
    ...
    for i in range(len(cores)):
        ...
        with open(f"{output_dir}/benchmark_idle_config" + str(core) + ".data", "wb+") as f:
            f.write(idle_configs[i].serialise())
