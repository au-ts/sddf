...
def generate(
    ...
    if board.arch == SystemDescription.Arch.X86_64:
        hpet_irq = SystemDescription.IrqMsi(
            board.arch, pci_bus=0, pci_device=0, pci_func=0, vector=0, handle=0, id=0
        )
    ...
    if board.arch == SystemDescription.Arch.X86_64:
        ...
        virtio_net_irq = SystemDescription.IrqIoapic(
            board.arch, ioapic_id=0, pin=11, vector=1, id=16
        )
        ethernet_driver.add_irq(virtio_net_irq)

        pci_config_address_port = SystemDescription.IoPort(
            SystemDescription.Arch.X86_64, 0xCF8, 4, 1
        )
        ethernet_driver.add_ioport(pci_config_address_port)

        pci_config_data_port = SystemDescription.IoPort(
            SystemDescription.Arch.X86_64, 0xCFC, 4, 2
        )
