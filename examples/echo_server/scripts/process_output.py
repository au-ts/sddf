...
colour_end = "\x1b[0m"
colour_codes = [colour_end, "\x1b[31m", "\x1b[32m", "\x1b[33m", "\x1b[34m", "\x1b[35m", "\x1b[36m"]
...
def output_and_reset():
    ...
    for core in range(num_cores):
        ...
        if not len(test["pds"][core]): continue
        test["active"] += 1
        ...
        for pd in test["pds"][core]:
            ...
            if pd_total_util > 1:
                print(f"Error - pd {pd["name"]} has CPU utilisation {pd_total_util} > 1")
            results_output["cores"] += result_string(
            pd["name"],
            total_core_cycles,
            pd["data"]["total"],
...
with open(file, "r") as f:
    for line in f:
        ...
        if match_total or match_pd:
            ...
            if "psci" not in pmu_key and "Model" not in pmu_key and "Load" not in pmu_key:
