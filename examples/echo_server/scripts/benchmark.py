...
def parse_args():
    ...
    # Note that by default we stop at 970Mb/s as the default packet size of 1470
    # means that the actual achievable throughput is 1470/(1470 + 40) as there is about
    # 40 octets of overhead for each packet due to 20 octets of overhead for each
    # ethernet packet plus 12 octets of IP, plus eight octets of UDP overhead.
    default_throughputs_mbs = [
        10,
        20,
        50,
        100,
    ...
    default_throughputs = [t * 1000 * 1000 for t in default_throughputs_mbs]
    parser.add_argument(
        "--throughputs", nargs="+", type=int, default=default_throughputs
    )
