...
// OUTCOME: With priorities, verifies in 0.01 seconds
// Without priotities, also verifies in 0.74 seconds. This is because all notify flags are always set to 1. With
// the simplified client, the CLT TX free notify flag is never changed (as it was only changed depending on the 
// size of the linked list). Therefore the MUX_TX also never changes the ETH TX free notify flag, and the ETH never
// changes flags anyway. The used flags are also never changed.
        ...
        :: skip ->
            if
                ...
                :: CLT_TX_free.notification ? 1 ->
                    ...
                    d_step {
                        do
                            // While there are packets to transfer from tx free in room in tx used
