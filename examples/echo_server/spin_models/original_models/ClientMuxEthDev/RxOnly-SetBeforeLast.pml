...
// DEADLOCK SOLUTION: Consumer sets flag to 1 before processing last packet in ring, while still in ring loop
// OUTCOME: 
        ...
        :: skip ->
            if
                :: CLT_RX_used.notification ? 1 ->
                    ...
                    if 
                        :: EMPTY(CLT_RX_used) -> CLT_RX_used.notify_flag = 1;
                        :: else;
                    fi
                    ...
                    d_step {
                        do
                            :: !EMPTY(CLT_RX_used) ->
                                if
                                    :: SIZE(CLT_RX_used, CHAN_LENGTH) == 1 ->
                                        CLT_RX_used.notify_flag = 1;
                                    :: else;
                                fi
...
active proctype MUX_RX() priority 1 {
    do
        ...
        :: CLT_RX_free.notification ? 1 ->
            ...
            bit notify_eth = 0;
            if 
                :: EMPTY(ETH_RX_used) -> ETH_RX_used.notify_flag = 1;
                :: else;
            fi
            ...
            d_step {
                do
                    :: !EMPTY(ETH_RX_used) ->
                        if
                            :: SIZE(ETH_RX_used, CHAN_LENGTH) == 1 ->
                                ETH_RX_used.notify_flag = 1;
                            :: else;
                        fi
            ...
            if 
                :: EMPTY(CLT_RX_free) -> CLT_RX_free.notify_flag = 1;
                :: else;
            fi
            ...
            d_step {
                do
                    :: !EMPTY(CLT_RX_free) ->
                        if
                            :: SIZE(CLT_RX_free, CHAN_LENGTH) == 1 ->
                                CLT_RX_free.notify_flag = 1;
                            :: else;
                        fi
...
active proctype ETH() priority 1 {
    do
        ...
        :: ETH_RX_free.notification ? 1 ->
            ...
            if 
                :: EMPTY(ETH_RX_free) -> ETH_RX_free.notify_flag = 1;
                :: else;
            fi
            ...
            d_step {
                do
                    :: !EMPTY(ETH_RX_free) && !FULL(HW_RX, COUNT) ->
                        if
                            :: SIZE(ETH_RX_free, CHAN_LENGTH) == 1 ->
                                ETH_RX_free.notify_flag = 1;
                            :: else;
                        fi
