...
// maybe transmmit data on the control connection
    ...
    while (q != NULL) {
      ...
      while (i < n) {
        if (ctrl->rx_phase == CTRL_WAIT_STATE) {
          ...
          sddf_printf("[%s] server state byte = %u (0x%02x)\n",
                sddf_get_pd_name(), ctrl->server_state, ctrl->server_state); 
          ctrl->state_ready = true;
          i += 1;
          
          if (st == PARAM_EXCHANGE) {
            const char *json = "{\"tcp\":true,\"time\":5,\"parallel\":2,\"omit\":0}";
            ...
            ctrl->num_streams = 2; // hardcoded for now
            ...
            for (int s = 0; s < ctrl->num_streams; s++) {
                ...
                error = tcp_connect(stream_pcb, &ctrl->server_addr, ctrl->server_port, iperf3_stream_connect);
            ...
            for (int s = 0; s < ctrl->num_streams; s++) {
                iperf3_stream_t *stream = &ctrl->streams[s];
                sddf_printf("STREAM[%d]: bytes=%llu\n",
                  s,
                  (unsigned long long)ctrl->streams[s].bytes);
                  total_bytes += stream->bytes;
            }
            sddf_printf("[iperf3] Test complete, total bytes transferred: %llu\n", ctrl->bytes_sent);
            // send fake results json for now
            const char *json =
              "{\n"
              "  \"cpu_util_total\": 0.67,\n"
              "  \"cpu_util_user\": 0.45,\n"
