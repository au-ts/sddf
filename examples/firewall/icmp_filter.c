...
// This is just a dummy filter that copies all packets to the router.
...
void init(void)
{
    ...
    sddf_dprintf("num buffers for rx: %d\n", net_config.rx.num_buffers);
    sddf_dprintf("This is the size of the active rx queue: %u\n", (uint16_t)net_queue_length(&rx_queue.active));
