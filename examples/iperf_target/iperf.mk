...
IPERF_SERVER:=${SDDF}/examples/iperf
...
SYSTEM_FILE := iperf.system
IMAGE_FILE := loader.img
REPORT_FILE := report.txt

include ${SDDF}/tools/make/board/common.mk

IPERF_SERVER := ${SDDF}/examples/iperf_target
METAPROGRAM := $(IPERF_SERVER)/meta.py
LWIPDIR := network/ipstacks/lwip/src
BENCHMARK := $(SDDF)/benchmark
UTIL := $(SDDF)/util
ETHERNET_DRIVER := $(SDDF)/drivers/network/$(NET_DRIV_DIR)
ETHERNET_CONFIG_INCLUDE := ${IPERF_SERVER}/include/ethernet_config
...
vpath %.c ${SDDF} ${IPERF_SERVER}

IMAGES := eth_driver.elf iperf.elf benchmark.elf idle.elf \
...
IPERF_OBJS := iperf.o utilization_socket.o \
	     udp_echo_socket.o iperf_tcp.o

DEPS := $(IPERF_OBJS:.o=.d)

all: loader.img

iperf.elf: $(IPERF_OBJS) libsddf_util.a lib_sddf_lwip_echo.a
	$(LD) $(LDFLAGS) $^ $(LIBS) -o $@
...
$(SYSTEM_FILE): $(METAPROGRAM) $(IMAGES) $(DTB)
	...
	$(OBJCOPY) --update-section .timer_client_config=timer_client_client0.data iperf0.elf
	$(OBJCOPY) --update-section .net_client_config=net_client_client0.data iperf0.elf
	$(OBJCOPY) --update-section .serial_client_config=serial_client_client0.data iperf0.elf
	$(OBJCOPY) --update-section .timer_client_config=timer_client_client1.data iperf1.elf
	$(OBJCOPY) --update-section .net_client_config=net_client_client1.data iperf1.elf
	$(OBJCOPY) --update-section .serial_client_config=serial_client_client1.data iperf1.elf
	$(OBJCOPY) --update-section .lib_sddf_lwip_config=lib_sddf_lwip_config_client0.data iperf0.elf
	$(OBJCOPY) --update-section .lib_sddf_lwip_config=lib_sddf_lwip_config_client1.data iperf1.elf
	touch $@
