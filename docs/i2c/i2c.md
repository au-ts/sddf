...
sDDF has support for I<sup>2</sup>C host controllers and interfaces for a small number of
peripherals.
...
I<sup>2</sup>C (inter-integrated-circuit) is a two-wire synchronous serial bus used for interfacing
...
I<sup>2</sup>C commonly is used to index into registers in a target. The protocol itself provides no mechanism to do so,
...
The I<sup>2</sup>C protocol abstracts I<sup>2</sup>C transactions into a series of *commands*.
...
An I<sup>2</sup>C transaction is a series of commands that must occur on the bus in a batch
...
which specifies the I<sup>2</sup>C bus address and number of commands in the transaction. The
...
Clients interface with the I<sup>2</sup>C virtualiser via a **request** and **response** queue.
...
libi2c provides a simple interface to perform blocking I<sup>2</sup>C operations using coroutines to
manage concurrency. It supports `libco` and `libmicrokitco` coroutines, defaulting to `libmicrokitco`
unless `LIBI2C_RAW` is defined at compile time. As with all I<sup>2</sup>C commands, data buffers
...
The I<sup>2</sup>C virtualiser is responsible for validating that incoming transactions are well-formed
...
The I<sup>2</sup>C driver is implemented as a finite state machine (FSM) using a generic structure.
