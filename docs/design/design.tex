...
\section{Asynchronous serial buses}\label{s:cl-serial}
...
UART tends to be much lower bandwidth (most serial ports have a
maximum bit rate around 4Mb/s; the POSIX standard only mentions up to
32768b/s.

As such the UART framework merges the data being transferred into
...
responses relative to requests. Each PD in the driver subsystem is connected
to its neighbours using a request and response queue, where requests contain
...
The data region only contains unstructured buffers supplied by the client and
nothing else.
...
an error value and a number of bytes written or read.
...
\mathrm{libi2c} is a simple blocking interface to the \gls{i2c} driver subsystem, supplying
functions to compose single-command transactions. There are three methods available:
\begin{itemize}
  \item i2c_write. Create a write of up to 256 bytes with a stop condition.
  \item i2c_read. Create a read operation of up to 256 bytes with a stop condition.
  \item i2c_writeread. Read from the register reg_address in the function call.
    Otherwise identical to i2c_read.
\end{itemize}

All libi2c calls will block the client until a response arrives. It supports the
seL4 Microkit coroutines library as well as as the generic \mathrm{libco} library used in
...
Unlike most other sDDF virtualisers, the \gls{i2c} virtualiser is an active
protection domain which executes freely instead of being driven purely
by incoming notifications. Incoming work from clients is enqueued in an internal queue structure
...
The virtualiser processes work from clients using a simple finite state machine.
\begin{itemize}
  \item Find header. Throw out all requests until a valid header is found.
    Invalid headers trigger a response to be generated for the client, while non-header
    commands are dropped silently. We can throw away non-headers as they are either
    erroneous or belonged to an invalid header. Once found, go to next state. If the header
    has more commands than can fit in the driver queue, we add this work to the internal
    deferred work queue and give up.
  \item Validate request. Check each command one at a time to ensure that flags and
    user-supplied buffer are valid, then translate buffer address to driver address space.
    Copy valid commands to an intermediary buffer.
    If any invalid commands are encountered, we throw away the intermediary buffer
    and return an error response to the client. If we don't find as many commands
    as the header indicates, we also consider this to be an error and return.
  \item Forward request. Once the entire request is validated and translated, the
    virtualiser enqueues the entire contents of the intermediary buffer to the driver
    and signals the driver.
\end{itemize}
...
the request or response queues. The driver maintains a global state structure
which we refer to as the "driver data" to avoid confusion with the finite state machine.
