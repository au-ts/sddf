...
char *client_names[SERIAL_NUM_TX_CLIENTS];

#endif

serial_queue_handle_t tx_queue_handle_drv;
serial_queue_handle_t tx_queue_handle_cli[SERIAL_NUM_TX_CLIENTS];

#define TX_PENDING_LEN (SERIAL_NUM_TX_CLIENTS + 1)
typedef struct tx_pending {
    uint32_t queue[TX_PENDING_LEN];
    bool clients_pending[SERIAL_NUM_TX_CLIENTS];
    uint32_t head;
    uint32_t tail;
} tx_pending_t;
...
static void tx_pending_push(uint32_t client)
{
    ...
    assert(tx_pending_length() < SERIAL_NUM_TX_CLIENTS);
...
void tx_return(void)
{
    ...
    uint32_t client;
    bool notify_client[SERIAL_NUM_TX_CLIENTS] = { false };
    ...
    for (uint32_t client = 0; client < SERIAL_NUM_TX_CLIENTS; client++) {
...
void init(void)
{
    ...
    for (uint32_t i = 0; i < SERIAL_NUM_TX_CLIENTS; i++) {
